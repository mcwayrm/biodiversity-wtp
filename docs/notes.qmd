...
> Jovin, Ryan, Matt, Raahil

## TODO: 
- Push github edits for Ryan (Matt)
- Tourist vs. Resident (Matt/Ryan)
- Update results for Above sample (Matt)
- Predict the panel WTP estimates (Matt)
- Updated figures for stylized facts (Ryan)
- Summary stats on rarity and apperance (by mean, IQR, max) (Ryan)
- Rarity/Apperance regressions (Ryan)
- Attempt to re-write for panel estimates = Equation 6 with \beta_{i,t} propogates through model. (Ryan/Raahil)

## Agenda: 
- Stylized facts and motivating facts (Ryan)
    - Added some figures and completed them
    - The apperance index may be constructed wrong.
    - For the new rarity historgram, bin the tail (truncate)
    - Redo this figure without the SD. 
        - We could plot the within-user means of expected richness, rarity, and apperance index over time (month, years)
        - Update the labels to match seasons for results table. 
    - For appereance, need to show sum stats for: 
        - Mean, IQR, Max
        - Show there is variation here. 
    - For hotspot seasonality, 
        - Make a new figure. 
        - Cummulative vists by month-year. Stacked filled area chart by migrant and resident. 
        - Should have a time series of avg controls/choice variables by user for selected sites and non-selected sites
            - Idea: What are the attributes they are selecting on average. 
    - Style Notes on figures: 
        - Font size bigger
        - Make the histogram dimensions more squarish
- Adding apperance and rarity 
    - Simple table 4 columns (or figure)
    - All measures Adding in apperance and rarity
    - Then all 3 seperately. 
- Regression
    - Need to add something explict estimating difference of tourists vs. residents
    - Need to update the results so "Above" is the main results presented in the manuscript
...
## TODO: 
- Complete stylized facts (Ryan)
- Create rarity and apperance results table. (Ryan)
- Troubleshooting adding new data (Matt)

## Agenda: 
- Controls: Group size and log-trips (Matt) 
    - This is incremental (experience - learning)
        - https://www.pnas.org/doi/10.1073/pnas.2013568117
    - Added! Completed!
- For historgram of species richness should be within individual IDs (Matt)
    - Updating so *within* individuals
    - They are correctly constructed. 
- Stylized facts and motivating facts (Ryan)
    - **PRIORITY**
- Adding apperance and rarity 
    - Simple table 4 columns (or figure)
...
## Agenda: 
- Create the main 3 panel figure displaying the core WTP results (Matt)
    - Figure Completed!
    - We want to add a panel with decomposition of species richness, rarity, and apperance index. 
        - By resident and migrant (what about these products is driving the demand)
- Change the reference season to summer (Matt)
    - Completed reset 
- Try an estimate without site-season FE (Matt)
    - Completed!
- Potentially control for group size since that may be conflated with congestion (Matt)
    - Not re-run yet. 
- Adding a control for dynamic learning/experience (Matt)
    - Increments by 1 each time they have a trip. 
    - Add as a fixed effect for bins of quintiles binned.
- Read about concerns with static income variable (GDP) -- all we need is time-varying travel cost & finish theory section (Raahil)
    - Completely fine. So no edits required from Jovin. 
    - Can forward fill GDP to later years. 
- Writing the beats of the paper: 
    - Behavioral model of revealed preferences
    - Construction of the data 
    - Reporting the core results 
        - Table of robustness for the appendix (different FE)
            - Individual
            - Individual by year
            - Site by season
            - Site (no season)
            - Try stable sample: Only keep baseline users at 2015 (accounting for composition changes)
            - Skill-level (frequency of usage) for users. 
        - Table A2 from Kolstoe mimic
        - - Want to report the within R2 for the regressions as indicator of variation after accounting for FE (Matt) --- Later
    - Identificaiton concerns (IV)
    - Expand on welfare implications
- For historgram of species richness should be within individual IDs (Matt)
    - Done.
- Add as stylized fact that individuals are choosing from a variety of hotspots. E.g., the SD of species richness across choice sets is large (Ryan)
    - Two sets: Motivational facts and discussion of what is going in the results. 
    - Motivational Facts (motivates decisions): 
        - Fact 1: Demand fluctuates with seasonal supply. And the spikes get larger as bird watching increases (demand is increasing)
            - Jovin figure
        - Fact 2: Supply fluctations are specific to migratory species in migratory hotspots. 
        - Fact 3: For individuals, they are selecting over diverse set of choices rather than a specific favorite hotspot. 
            - Matt histogram
        - Fact 4: Are individuals selecting hotspot locations on species richness, rarity of species, or charismatic species.
        - Fact 5: We have heterogeneous preferences. E.g,. why the controls matter and result in variation in WTP on these attributes. 
    - Sytlized Results (explains variation): 
        - 
- Need to add a rarity index for species and popularity index (Ryan/Jovin/Matt)
    - Update script `07_compute_biodiversity_metric.R`
    - Build the indices 
        - Ryan making rarity (probability metric)
        - Joving making the apperance index which provides mean, SD, max
    - Then add them as a scenario version  
...
- Read about concerns with static income variable (GDP) -- all we need is time-varying travel cost & finish theory section (Raahil)
    ...
    - Can forward fill GDP to later years. 
- Change the reference season to summer (Matt)
    - Has been reset for seasonal reference. 
- Potentially control for group size since that may be conflated with congestion (Matt)
- For historgram of species richness should be within individual IDs (Matt)
- We also want to report the within R2 for the regressions as indicator of variation after accounting for FE (Matt) --- Later
- Add as stylized fact that individuals are choosing from a variety of hotspots. E.g., the SD of species richness across choice sets is large (Ryan)
    - Two sets: Motivational facts and discussion of what is going in the results. 
    - Motivational Facts (motivates decisions): 
        - Fact 1: Demand fluctuates with seasonal supply. And the spikes get larger as bird watching increases (demand is increasing)
...
- Need to add a rarity index for species and popularity index (Ryan/Jovin/Matt)
    ...
    - Build the indices 
        ...
        - Joving making the apperance index which provides mean, SD, max
    - Then add them as a scenario version  
- Need to come up with how the results are presented (Ryan)
...
- Need to add a rarity index for species and popularity index (Ryan/Jovin/Matt)
- Need to come up with how the results are presented (Ryan)
- As a group, we need to do an e-bird field trip to understand this better (All)

## Agenda: 
- Updated results (Matt)
    - De-standadrize coefficients make WTP bigger
    - Fall is the higher (This is good. We expect this now.)
- Updated travel cost transfer (Jovin/Matt)
    - We have updated GDP data now. Part is only updated through 2022
    - Still don't go through 2024.
    - Can use GDP inflation to forward fill static income variable
    - We have individual fixed effects, so we can use the individual fixed effects to account for income differences across individuals/regions.
        - Probably site-season fixed effects will also account for income differences across regions.
- Results presentation: 
    - First WTP overall.
        - This is small. Likely because they care about specific species (migrants). So they discount overall biodiversity to seek out specific species.
    - Then migrant vs. resident species richness. 
        - Ideally positive on migrant and zero on resident. 
        - All the demand is being driven by the migrant species. Demand is driven by scarce natural amenity. 
    - Then open it up to seasonal effects. 
        - Migrants should be most abundant in winter. 
        - Actually, the influx of migrant birds come in Sept. - Nov. Then they stay for 4 - 5 months. This is the nesting ground so the birds are hard to see in the winter while protecting babies.
        - So, the scarce ameniety becomes abundant in a specific window in the fall. This is the time for consumers to purchase when abundant in the market.
    - Figure 1: 
        - Panel A: Value of species (average overall)
        - Panel B: Species Types (migrant vs. resident)
        - Panel C: Seasonal Effects (Each season with two points for migrant vs. resident)
- Histogram of biodiversity over choice sets (Matt)
    - What variation we are working with. 
    - Currently have across choice sets rather than within individual
    - We want the histogram of the SD for each individual. 
        - For each person, what is distribution of species richness across choice sets
        - Save the SD for each individual
        - Plot the distribution of SD. 
- Line graph of user visits by month and migrant hotspots (Jovin)
- Continue writing (Raahil)
- Incoporating species level appearance index (Jovin)
    - AVONET data to match on species. But probably not good for looking at attractiveness of the bird.
    - Jovin found a new e-bird driven visual aesthitic attractiveness index rated by humans
    - We want at hotspot level, Max, SD, and max attractiveness by migrant and resident. 
- Expand on how to do the control funciton efficently (Ryan)
...
> Jovin, Ryan, Raahil, Matt

## Updates: 
- Map specific tests to each explanation (Ryan)
    - Supply and demand offsetting (compensating differential) 
        - Logic: high-biodiversity sites already have offsetting costs (congestion, crowding, access restrictions, entrance fees) baked in, so coefficient understates true WTP because biodiversity is already "priced" through other channels.
        - A version of model with / without the site-season FE
        - Can use the IV because we cannot unbundle WTP from other unobserved determinates (in the epsilon)
        - Counterargument: Site-season FE should have already accounted for this. Show model with successively adding FE (show a big difference in coefficient). 
    - Seasonal Migration Demand
        - Logic: Users trade off overall biodiversity to seek out rarer species (migrants) with seasonal inflow of supply  
        - Site-Season FE should have already accounted for this
        - Congestion is another control that should hold demand constant
        - A historgram of relative migratory species captured should spike during winter while resident species visits are relatively flat. 
        - If the average preference for residence is constant over time. And the sites that have migratory species should spike in migratory months. 
            - Line graph with three lines: migrants at migrant sites, residents at migrant sites, residents at resident sites.
    - WTP is actually low
        - Logic: Envirodevonomics explantion of environmental goods as luxury goods.
        - If we get individual WTP, then we can map this back to the gridded data. 
        - Can observe does high WTP map onto the income. 
        - Could try to show this with an Engle curve
    - Charasmatic species instead of biodiversity. 
        - Logic: Biodiversity is the nature attribute driving demand. This is omitted variable bias.
        - An index of the fancy bird vistations by hotspot. 
            - Could try to do a physical apperance index from e-bird
                - Can consult with Colleen on ornothology
                - https://opentraits.org/datasets/avonet.html
            - Since data driven, is it actually rare or just undesirable.
            - Should look up species rarity index. 
        - Additionally, we can create a rarity index for each species. What are the least likley species to be observed in India. So hotspots with the highest likelihood of observing a rare species may drive specific users. 
        - Add this into the model as an attribute. This may absorb the biodiversity effect.
            - Will need to have counteracting effects. 
            - So will rarity/charasmatic species postive or negatively correlated to biodiversity (does biodiversity crowd out)
    - Insufficient identifying variation
        - Logic: Too little variation to identify the effect of biodiversity on WTP attenutating effect size.
        - Solution, successively add in FE to see what changes. 
            - Empirically, what is the within R^2 at each point. 
        - Would like to see the variation/distribution of biodiversity
            - Summary Stat figure which is SD of biodiversity within choice sets (histogram). 
            - Historam of expeceted diversity within an individual across choice sets.
    - Artificial biodiversity Substitues (TV documentaries) -- Leave this one. 
        - Logic: Subsistution effects drive down WTP
        - The data starts in 2015 so we cannot do internet/TV expansion (also correlated with our measure of income)
        - The closest we could speak to this would be some measure of nature documentary viewership.
        - We could control for distance to nearest zoo 
    - Sample selection on the extensive margin
        - Logic: eBird users who already selected into birding — a group that plausibly has near-maximal WTP for biodiversity already baked into their choice to be birders at all.
        - Untestable unless we get a different set of users for comparison.
        - DROP THIS. The identifying variation comes from switching between sites, not the extensive margin.
    - Reference dependence/novelty decay.
        - Logic: Users may have a reference point for biodiversity and may not value additional biodiversity beyond that reference point.
        - Test by restricting the sample of observations with first time a user visits a given site. 
        - How many unique hotspots does a user visit? Histogram.
- MODIS 2024 data transfer (Raahil/Matt)
    - DONE. 
- Stylized Facts section motivating the model (Ryan)
    - Stylized Fact 1: Who are these users?
    - Stylized Fact 2: Hotspot visitation fluctuate seasonally.
    - Stylized Fact 3: Biodiversity varies across a user's choice set.
- Updated travel cost transfer (Jovin/Matt)
    - DONE. 
    - Matt re-running pipeline to incorporate it.
- Line graph of user visits by month and migrant hotspots (Jovin)
    - Comparing number of user visist seasonally by sights that only see residents compared to sites that ever see migrants. 
        - Comparing above and below median of migratory or resident species.
    - Steps:  
...
1. Updating travel cost data (expand to 2024 and average cost across the four cities) (Jovin)
    - We should be finalized on this. 
    - 3 cities: Delhi, Chennai, Mumbai
    - Average travel cost decreased to 140.49 rupees on average
    - Range: 2015 - 2024
2. Updating the results with expanded years and updated travel cost (Matt)
    - No complete run with the new cost data. 
        - Code is updated, but need the updated input data for travel cost measures. 
        - Need to re-download. 
    - Need to updated MODIS data for 2024 (Raahil)
        - % Tree cover. 
        - Already have other 2 ERA variables for the full time frame. 
        - Can currently update for 2023.
3. Incorporating the IV in the regression output (Ryan)
    - Successfully ran the whole pipeline through. 
    - Incorporated script 10 for the compute IV
    - Will need to bootstrap the IV to get the correct standard errors. 
    - Need an effecient way to do this given the long run time for the xlogit.
4. Need to add a graphic of the demand changes by month over time (Jovin)   
    - Explains one of the explanations for the WTP measures
    - Could be the demand changes with time also
    - Will make a line graph of user visits by month and migrant/resident. 
5. Think through the story of the results (All)
    - Potential explanations: 
        - Supply and demand are off setting each other. 
            - What is exactly is the variation we are identifying off considering the controls and FE?
            - A version of model with / without the site-season FE. 
            - But even with FE you can violate parallel trends in other ways (motivates IV)
        - Demand during migration months 
            - Can we hold the demand curve constant. 
            - Site-Season FE should have already accounted for this
            - Congestion is another control that should hold demand constant
        - IV creates exogenous variation in expectation
            - So, an outside variation in exected utility. 
            - Can use the IV because we cannot unbundle WTP from other unobserved determinates (in the epsilon)
        - Perhaps WTP is actually low
            - Potential issue: we impute wealth so we don't observe it direlty. 
            - This would be consistent with envriodevonomics literature that envrionmental quality is a luxury good. 
            - But perhaps recreation is not culturally desriable. So closer to an inferior good relative to the outside option (staying at home to watch TV). 
        - Folks do not actually care about biodiversity but rather just charasmatic species. 
            - So, only seeking out places with very attractive species. 
            - Could make an index of the fancy bird vistations by hotspot. 
            - Sort of already observed by seasonality of WTP
            - Could try to do a physical apperance index from e-bird
                - Can consult with Colleen on ornothology
                - https://opentraits.org/datasets/avonet.html
        - Perhaps artificial biodiversity is avaliable at home
            - At low cost, can see diverse species with a documentary instead of searching with hit/miss at hotspots. 
        - We may have too little identifying variation
            - All the fixed effects are soaking up all the variation. So very little to identify off of. 
            - If different sites are very similar within states, then very little to identify off of. 
            - Solution, successively add in FE to see what changes. 
            - Would like to see the variation/distribution of biodiversity
                - Summary Stat figure which is SD of biodiversity within choice sets (histogram). 
                - Historam of expeceted diversity within an individual across choice sets.
6. Plan for the remainder of the summer (All) 
    - Potentially doing working sessions this summer. 
...
1. Updating travel cost data (expand to 2024 and average cost across the four cities) (Jovin)
2. Think through the story of the results (All)
3. Updating the results with expanded years and updated travel cost (Matt)
4. Incorporating the IV in the regression output (Ryan)

## Current Story of the Results: 

1. Postive WTP overall (ecosystem is valuable)
2. This is suprisingly low. This is because willing to forgo overall species richness to seek out specific species. 
3. If we decompose by season x migratory species, we observe this.  
4. One explanation of the issue, is the supply and demand issue. In winter, there is an increase in supply of species. Currently working on examining shift in demand curve by looking at season user activities. 
5. This motivates holding demand constant through a supply shifter. E.g., \beta is endogenous. 
6. Bird flu only affects migratory species (e.g., LATE is only for migrants)

## Agenda: 

1. Write up the current state of the analysis in the data section (Matt)
    1. Mainly completed. Some numbers are missing. 
    2. Hotspot clusters (rename) recreation areas. 
    3. Mostly completed the variable construction write up. 
    4. Added some basic plots
2. Need to select a user group for the main paper (All users, above/below median) --- DONE
    1. Full sample 
    2. Counterfactual is 10 alternative sites randomly sampled within X KM hotspot clusters
3. Travel cost update (Jovin/Matt) --- DONE
    1. Has been updated in the code and the overleaf. 
        1. Issue before was a currency exchange issue. 
    2. We have travel cost info through 2021. 
    3. So need to update 2015 - 2024. 
    4. Will require extending the MODIS data through 2024 (currently limited to 2023)
    5. Currently the petrol prices using only Delhi. Will need to average across the prices. 
    6. Add disclaimer that petrol prices are averaged across 4 major cities in India. 
4. Updated Results: 
    1. The coefficient size is still very, very small. 
    2. Seasonality matters. In winter, visitors sacrifice diversity to seek out specific species. 
    3. Make a graph of users by season and year. So does winter migrants drive a growth in consumer demand. 
        1. And number of users going to hotspots with migratory species specifically. 
        2. Can be done as a regression as well. # of users on dummy for seasons and interaction with migratory species exposure. 
        3. Explain why we observe decreased WTP in winter. 
    4. For now, we can stick to just the full sample (above/below for the appendix)
    5. Can try individual and year FE instead of individual x year FE. 
5. Zero market share thing (Ryan / Jovin)
    1. Our study problem: 
        1. Do we have any site-time observations where noone selects the site (e.g., no visits)
        2. We need to remove these choices from the choice set. 
        3. https://users.ssc.wisc.edu/~xshi28/research/gandhi_lu_shi.pdf
        4. https://www.econstor.eu/bitstream/10419/316152/1/1922854557.pdf
    2. Methods: 
        1. Earle and Kim aggregate until no zero market. 
        2. Dubé et al. introduce a selection function that removes alternatives from the choice set when they have a zero share
        3. Li applies an empirical Bayes procedure to estimate strictly positive shares using information from other markets.
    3. Resolution: 
        1. This is not an issue for us. 
        2. Our counterfactual hotspots are consumer driven. So this is never zero (by definition) 
6.  Incorporating the IV (Ryan)
    1. Check the control function used in Wildfire paper
    2. Or attempt boostrapping the two stages
7. Explore how to estimate individual WTP from mixed logit (Raahil)
    1. mlogit allows for this. 
    2. `individual_params <- fitted(mx, type='parameters')`
    3. `mx` stands for mixed logit. 
    4. $g(\beta | \theta)$ = population distribution of WTP by dispersion paramter. 
    5. We can use the extra moment of the distribution to assign individuals to a place in the distribution given other characteristics of the individual. 
    6. X creates a class of individuals which create a sub-distribution within the \theta distribution. 
    7. Use observed behavior to determine where you are in the distribution given the dispersion parameter. 
    8. g() is the prior belief. Then we update the belief given observation X to construct posterior belief.  
    9. A cross-section of individuals demand across all their trips to create individual WTP. $h(\beta_i, \theta, X) = \underbrace{Pr(y_i|\beta, X)}_{Likelihood} \times \underbrace{g(\beta | \theta)}_{Prior}$
    10. Add $t$ to $i$ to create the panel. 
    11. The more $X$ data the better -- better characterizes the individual. 
...
2. Need to select a user group for the main paper (All users, above/below median) --- DONE
3. Travel cost update (Jovin/Matt) --- DONE
4. Zero market share thing (Ryan / Jovin)
    1. Provide a step by step mapping of the problem and solution to our setting. 
    2. Summarizing the issue and discussion in the literature
5.  Incorporating the IV (Ryan)
    1. Check the control function used in Wildfire paper
    2. Or attempt boostrapping the two stages
6. Explore how to estimate individual WTP from mixed logit (Raahil)
    1. Similar to post-command in Stata
    2. Kenneth Train textbook section on math and logic of this
    3. We will have a user-year panel of WTP 



## Agenda: 

1. Estimation Updates
    - Estimated mixed logit with python package
        - xlogit
        - What are the $\mu, \sigma$ from the model
            - It is okay if $\sigma$ is negative: https://stats.stackexchange.com/questions/158260/mlogit-negative-value-for-sd
        - Add expected richness, drop summer season, and only make expected richness as the random coefficient
    - For migrant residents: 
        - Do a version with # of unique species (richness) and a version with county of total species (abundance)
    - Resident richness is dropping because colinear with total richness
        - Try a version of the model that drops total richness and only keep resident and migrant richeness
    - Some evidence that high users are trading off species richenss to seek out migrant richness. This is almost entirely happening during the winter season.
    - Can attempt redefining the user frequency. 
        - Have below/above the median. 
    - For fixed effects, we are trying several 
        - Currently, individual, individual-year, year, hour-of-day
            - Think of hour FE as accounting for agglomeration of supply by species
        - Could create an alternative control variable. 
            - Cummulative experience by user as a fixed effect (total number of trips up to that trip in t)
            - We want to detect if some users are better at detecting species and not necessarily higher demand for species
2. Zero Market Share Issue: 
    - For us: 
        - Ensure that the choice set for an individual is consistent 
        - We need to justify if this matters: We are not truncating nor are we aggregating. 
        - Useful reference: https://le.uwpress.org/content/wple/early/2026/02/13/le.102.3.013026-0020.full.pdf
        - This paper justifies that we can use our control variables as a control function to resolve this problem if it exists: https://www.theigc.org/sites/default/files/2016/06/READINGS-Levinsohn-Petrin.pdf
    - "Esimtating Demand for differentiated products with zeros in market share data" Gandhi, Lu, Shi, Quantitative Economics [Link](https://onlinelibrary.wiley.com/doi/epdf/10.3982/QE1593)
        - Zero demand is common in observed differentiated product markets
        - Impact on the conditional expectation of the inverse demand
        - Large data leads to sparsity in discrete choice. 80/20 rule 80% of consumption by 20% of the products
        - Conditioning on a time window (i.e., week) a large majority of products are never selected. 
        - This is directly related to Zipf's power law in patterns of demand.
        - The issue arises in the model from equation 5 in the paper on the substitute parameter
        - Section 3.2 describes the problem as $log(s_jt) \to -\infty$  when $s_jt = 0$
        - This is selection on observables issue and leads to attenuation bias.
    - "Random-coefficients logit demand estimation with zero-valued market shares" Dube, Hortacsu, Joo. 
        - Many purchase data sets have high incidences of products with zero sales
        - Berry 1994/1995 allows for endogenous discrete choice models assuming that consumers make non-zero choice probabilities over each product.
        - Markets that observe zero market share for some products defies this assumption.
        - This leads to adverse selection in the data
            - Selection bias with sysetmatic modification or truncation of observations
        - Resolve with 'considertation instruments'
            - products that effecitvely considered by consumers, but are excluded from the consumption utilities.
3. Travel cost estimations being too low
    - Issue was that GDP in USD but cost measures in Ruppee. 
    - So this conflated some problems
    - So 2019 exchange rate for the GDP to ruppee
    - Finally WTP should be in Rupees
...
1. Random sample of choice set (Matt)
2. Decide which is the "main" sample (Matt)
3. Play with different fixed effects (Matt). Too many FEs may absorb all the useful variation and lead to the standard error issue
4. Final decision on mixed logit or conditional logit (Matt)
5. Consolidate lessons from the Land Economics issue into a half page lit review + gaps (Ryan)
6. Figure out why mean travel cost is so small - go through the code line by line (Jovin)
    - Travel Cost attenuation (Jovin)
7. Zero market share thing (Ryan / Jovin)
8. Set up meeting with Jacob Gellman for help on mixed logit (Raahil)
9. Continue writing (Raahil)
1. Incorporating the IV (Ryan)
...
1. Estimation Updates
   * WTP estimates with standard errors (Matt)
     * Issue with saving summary object (Hulton Draw matrix)
   * Estimates with various FEs (Site-by-season, hour-of-day, etc) (Matt)
     * One of the controls (distance to PA) as no variation within some choice sets. 
     * Some merge issues for migrant/resident richness. 
   * Variation: 
     * Endow variation by only picking choice set of 10 - 50
     * Literature supports a random sample converging to true population distribution.  
     * Jacob Gellman ([Wildfire smoke](https://www.journals.uchicago.edu/doi/abs/10.1086/732404)) could provide a meeting to discuss the issue. 
   * Mixed logit estimates (Matt, Ryan, Jovin)
     * Currently on a small sample (0.01)
     * Could switch to python instead of R
     * If computation is too expensive, will need to use MSI supercomputer.
2. Travel Cost
   * Revised travel cost calculation (Jovin)
     * Estimating driving cost (before was static of 7.5 IND/km)
     * Brings in petrol and travel data (2011 - 2021)
     * Kaggle and PPCC data
     * Use CPI deflator from World Bank 
     * Bring in static values for maintence cost and fuel efficiency
   * Issue: Current summary stats have travel cost of 3, not higher when including distance. (closer to a 100 times larger)
     * Somewhere there is an error. 
3. Sample
   * Identify main sample (I need this for DHS analysis)
     * Depends on results sensitivity to choice set selection. 
     * Need to fix the main sample for description in the sample soon. 
   * Fill in the biodiversity data section in overleaf (Section 4.2.3)
     * Hierachical selection of imputing information for computing expected richness. 
4. Writing
   * Land Economics special issue on rec demand with cellphone data (Ryan)
     * Need to summarize best practices and findings in the recreation demand literature (add to scratchpad)
       * What are the gaps in the literature
       * Address the zero market share issue (https://le.uwpress.org/content/early/2026/02/13/le.102.3.013026-0020)
     * https://le.uwpress.org/
     * "Evaluating Mobility Data for Recreation Monitoring: Diagnostics and Implications for Best Practice"
       * https://le.uwpress.org/content/wple/early/2026/03/05/le.102.3.011626-0006.full.pdf
       * Discusses data generating process for mobile data
       * Identifies mediators for false negatives and false positives. 
     * "Are mobile device location data a substitute for travel cost surveys?"
       * https://le.uwpress.org/content/early/2026/03/05/le.102.3.012626-0015
       * Compares mobile location data to survey data in national parks
       * A very complicated measure of travel cost
       * Sort of estimates MWTP over time. 
       * Relatively consistent results
     * "Causal Recreation Demand Estimation with Cellphone Mobility Data"
       * https://le.uwpress.org/content/early/2026/02/13/le.102.3.013026-0020
       * Estimating Bayes posterior mean
         * Appears to be important to address "zero market share issue" e.g., sites in choice set with no observed visits within a given time frame -- a spare data issue
         * Estimate with beta-binomial model to impute likelihood of site demand  proposed by Li (2019)
       * Synthetic DID for choice set using beach closures
     * "Quantifying Recreational Benefits from Fish Consumption Advisories"
       * https://le.uwpress.org/content/early/2026/02/13/le.102.3.013026-0018
       * Fish advisories as exogenous shock >> causal IV using toxic release inventory upstream/downsteam
       * "We model recreational behavior based on a discrete-choice modeling approach following the framework of Berry (1994) and the two-step estimation technique outlined by Murdock (2006). In the first step, we use a random utility model to estimate recreation demand and recover the mean utility gain from visiting each recreational site across all visitors using site-specific alternative-specific constants (ASCs). In the second step, we regress ASCs estimated from the first step on site attributes, including the presence of FCAs, to estimate the impact of FCAs on recreation demand."
       * Following the common assumption in the recreation demand literature (Palmquist et al., 2010; Lupi et al., 2020), $TC = 2 (gas cost + vehicle maintenance) * distance + 2/3 (median income/2080) * travel time$
       * Includes seasonality
     * "Estimating the Economic Value of Grassland Tourism Services Based on Mobile Phone Data"
       * https://le.uwpress.org/content/early/2026/02/13/le.102.3.012726-0016
       * Phone signal locations data
       * Zonal travel cost 
       * KNN regression to generate choice set.
       * Estimate consumer surplus as integral of travel cost by population for origins
       * Following Jaung & Carrasco (2020), the standard formula is: $TC = \sum_n (portion by method) * transportation cost + 1/3 hourly wage * travel time$
     * "The Impact of Water Quality at the beach on disadvantaged communities in Rhode Island"
       * https://le.uwpress.org/content/early/2026/02/13/le.102.3.013026-0019
       * Cell phone locations
       * Zonal travel costs with improved water clarity 
       * Beach closure events as source of exogenous variation
       * Reduced form estimates.
       * "Following Merrill et al. (2020), we divide the number of visits by 3.58 to correct its overestimation in the cell phone data."
       * "The travel cost is calculated using travel distance and travel time. These were calculated using the Open-Source Routing Machine (OSRM) which estimates travel time and distance through a road network (Luxen and Vetter, 2011)"
   * Any updates on model organization (Ryan)
   * Paper figures
     * Currently we have placeholders
...
- Add distance to site, and choice set size, in the summary stats
- Show only WTP estimates overall and by season. Drop monthly estimates
- We can drop WTP for the other site attributes
- Add shannon and simpson indices. The computation should already be there in my original code. If not, let me know and I will send it.
- Count number of migrants and residents per trip. Also compute migrant species richness and resident species richness per trip
- Add migrants and residents to conditional logit equation. Also try interacting each with seasonal dummies
- Add site-by-season FEs
- Important: pilot the mixed logit estimation on a subsample of users. Try a random 10% of users to keep sample size low

2. Jovin

- Double check travel cost. 3 Rs average seems too low
- Have a look at this paper and see if we can use ES (equation 3) 

3. Ryan

- Meet with me regarding the theory
...
- **Matt**: Updated the scenarios and estimates given the discussion.
- **Ryan**: Adding IV to pipeline
- **Ryan**: Expand out contributions in the literature for presentation
- **Raahil**: Narrative about how preferences 
- **Jovin**: Adding documentation about data sources

## Plan for the semester (medium term milestone)

- Pin down the story
- Then proceed to IV estimates
- Leave the individual WTP for after medium term
- NatCap meeting sign up for late in the semester 
    - Completed estimates without the IV or individual WTP
    - March 30th 
    - Matt presentation

## Agenda

1. Raahil: Figure out steps for individual WTP
    - No pre-written script for this in R, but exists in Stata
    - So might need to call a Stata command from R. 
2. Seasonality to the utilty function
    - When expanding to 30KM, the WTP increases. 
    - Maybe do for 50KM.
    - About 18 scenarios estimated so far
    - Can remove the 5KM for now. 
    - Vary by new user, expert, and average user
3. Adding polynomial to species richness
    - Add $species^2$
    - Start by adding to one scenario
        - If it doesn't work in one, then won't be important in the others.
4. Adding IV to master data 
    - Incorporating Jovin's data to base data
    - New script to estimate IV and merge with master data 
    - Add regression run for IV
        - Because species richness is residual, then will need to bootstrap the residuals and estimate of WTP
    - Supply shifters may explain why winter is so low (even though there are high migration in winter)
        - So we need to account for Total along with marginal willingness to pay
        - And the IV will account for why WTP (price) drops when supply of species is abudnant (quantity)
        - In winter, you may not need to travel anywhere because species are so abundant they could be right outside your window (= 0 travel cost but still high value)
    - Case for IV: 
        - Migratory species over seasons by attenuate results
5. Presentation
    - How will be present the results cleanly
    - Ideally, MWTP and TWTP for species richness for the combination of these: 
        - User type: average (annually at least 2 years), veteran/super user (quaterly for 5 year), expert (bi-annually for 3 years)
            - Alternative, create a variable that is use rate (number of trips per year, number of months recording data)
            - Then look at a histogram with three quantiles (i.e., terciles)
            - Now it is based on the distribution of use 
        - Counterfactual space: 30 KM, 50KM
        - Use the fixed effects version (dropping basic)
    - Do you value the site or value the diversity at the site?
        - So the other variables are important place based effects
        - Congestion would be popularity
        - For tree cover, can be negative because birds hang on the edge of the tree ----------
        - Temperature is odd as highly positive. 
        - Percipitation makes sense
    - For interpertation: 
        - Number of trips in a year (so disperse cost over several trips)
        - Cost is relative to base income (absolute MWTP may be better interperted as a relative measure)
6. User Controls
    - User-trip info on duration of the trip AND distance walked
    - For distance walked:
        - Stationary trips: NA = 0
        - Travel trips: should have a value
    - So need these to control for value of time outdoors or exercise, rather than biodiversity
7. Contributions: 
    - Developing country setting
    - Supply angle (why it is low)
    - migration IV
    - Mixed logit
...
- **Matt**: Scenarios to add: seasonality and interaction of seasonality with month
- **Ryan**: Add polynomial of species richness into model
- **Jovin**: Write up documention to verify IV results and why this makes sense with flyway zone
- **Ryan**: Add the IV into the task flow


## Agenda

1. Matt: Attempt robustness checks (vary years, vary experience levels)
    - For scenarios: 
        - 3 types of users (new, medium, and experts determined from breakpoints in distribution of visits)
        - varying radius of household (5 to 30 alternative choices) 
        - model version with seasonality
    - Currently working on implmenting the scenario settings
        - Changing the choice set around household is crashing the logit estimation
        - Crashes at 30 KM, so proceeding with 10 KM
        - So only partial results currently
    - Can proceed with the conditional logit for now -- will return to mixed logit
        - Need mixed logit to estimate individual WTP
    - Results would be 3 x 3 grid for WTP on species richness   
        - One dimension: user experience 
        - One dimension: choice set size
2. Matt: Sanctity checks for running robustness checks 
    - Still working through all the robustness checks
3. Jovin: Completing constructing the shift so we can complete the shift-share IV
    - Raahil had already classified migratory species for some birds
    - Jovin is appending data for migratory classification on the remaining species
    - 410 new migratory species
    - In total, 1,228 migratory species
        - This is likely an over-estimation of the number of birds in India
        - Online search says roughly 1,400 birds (migratory and resident)
    - Have constructed $S_J, \gamma_j, shift_t$
    - So we have constructed $IV_{dt}$
    - Most of the exposure and volatility in the IV is in the northwest of the country.
4. Ryan: Re-running Matt's code structure from start to finish (code review)
    - Adaptable to any context
    - Just need some small tweeks to input data specific to any country or setting
    - Add this to the data section
5. Ryan/Raahil: Explore the logitr command (Raahil has an explanation in prior email)
    - We may be getting negative WTP because of user type
    - So experienced users are seeking a specific species (key stone species)
    - Less experienced users seek a broader diversity of species (biodiversity)
    - Actually don't know which type of user perfers biodiversity (Pos. WTP) vs targeting a key stone species (Neg. WTP)
        - Perhaps incedental observations are more of biodiversity rather than targeted species
        - Targeted species would only be from 'highlighted' report by birders
        - Targeted species may appear only with seasonal migration
            - We could estimate for only a specific month (compare Nov. to Nov.)
        - Interacting species richness with each month
    - Could create a polynomial for species richness
        - Add $species^2$
6. Raahil: Figure out steps for individual WTP
    - No pre-written script for this in R, but exists in Stata
    - So might need to call a Stata command from R. 
