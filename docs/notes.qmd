...
- Ryan: Get the correct e-bird file from Matt
- Ryan: Merge code files into scripts folder. (DONE)
- Ryan: Initate a document for literature review with template example (DONE)
- We need to specalize and define tasks. Volunteer over email. Tasks: Lit review, sample selection, characterize the sample, theory person
- And set up a recurring meeting time. Need a poll. Try for next two weeks from now.
- Jovin: Provide your github username so Ryan can make you a collaborator on the github repo. 
