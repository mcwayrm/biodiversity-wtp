...
- Here is a general critique on WTP measures being static and not accouting changes in wealth over time [article](https://www.science.org/stoken/author-tokens/ST-1741/full). We could speak empirically to this theoretical conjecture. 
...
*[JAERE (2026)](https://www.journals.uchicago.edu/doi/epdf/10.1086/741509)*  

- Incorporates variation over time (panel recreation model)
    - First, estimate site fixed effects -- the mean utility of site over time controlling for travel cost
        - $\delta_{jt}$ = mean utilities = site by choice occasion fixed effects
        - Follows from Murdock (2006) which applies Berry (1994) contraction mapping to minimize computational burden
    - Second, linear causal estimates of site attributes using synthetic control
        - Idea is to decompose the mean utilities
- Causal identification from beach closures in Lake St. Clair Metropark
- Travel cost using zip codes of visitors. 
- Structural synthetic controls as counterfactuals for welfare loss estimates
    - Welfare using compenstating varation (CV) measured through lost trips to closed beaches.
- Identify stigma effect: delayed welfare loss from temporary beach closure
- They do NOT estimate a dynamic model. Only consider contemperanous choice. 
- They mention congestion from site closures, but do not address it directly. 
- Zero market share get around: temporal aggregation. 
    - "Dubé et al. introduce a selection function that removes alternatives from the choice set when they have a zero share, and Li applies an empirical Bayes procedure to estimate strictly positive shares using information from other markets."
- We can gain efficiency if we apply the Berry contraction mapping. 
    - Slide 26: https://jfhoude.econ.wisc.edu/wp-content/uploads/sites/769/2023/10/3.-Aggregate-demand.pdf
    - Original Paper: https://www.its.caltech.edu/~mshum/gradio/papers/Berry1994.pdf
