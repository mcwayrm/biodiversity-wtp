...
# Dynamically find all wtp_*.rds files for each scenario
wtp_list <- list()
models <- c()
message("\n=== Loading WTP Data ===")
for (scenario in scenarios) {
  ...
  message("[WTP] Scenario ", scenario, ": Found ", length(wtp_files), " WTP files")
  for (wtp_path in wtp_files) {
    model_name <- sub("^wtp_(.*)\\.rds$", "\\1", basename(wtp_path))
    message("[WTP]   ", basename(wtp_path), " (model: ", model_name, ")")
    if (file.exists(wtp_path)) {
      ...
      tryCatch({
        ...
        if (!is.null(wtp_df)) {
          ...
          message("[WTP]     Loaded: ", nrow(wtp_df), " rows")
        } else {
          message("[WTP]     ✖ Could not parse WTP object")
        }
      }, error = function(e) {
        message("[WTP]     ✖ Error loading WTP: ", e$message)
      })
    }
  }
}
...
message("Available models: ", paste(models, collapse = ", "))
```
...
if (length(wtp_list) > 0) {
  ...
  wtp_combined <- wtp_combined %>% 
    ...
    mutate(model = factor(model, levels = unique(models)))
