...
samp <- 1                 # random sample of users (scale 0-1) -- Random sample 0.1% of users (test case)
...
trips <- readRDS(config$ebird_trip_hotspots_path) %>%
  ...
  stopifnot(nrow(trips) == 78593) # CHECK: Subset of Obs = 492,485
...
stack_cset <- function(i) { # TODO: Should this be it's own script? Actually we should have all the functions in one scripts
  ...
  if (module == "cluster") {
    id <- sample$cluster_id[sample$trip_id == i]
    user_cset <- filter(user_cset, cluster_id != id)
  } else {
...
# Save results
file_name <- paste0("master_cs", radius, "km_clust", clust_size, "km_sample", samp, ".rds")
