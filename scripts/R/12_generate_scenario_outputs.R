...
# Model results are in inputs$models_output_dir (CSV files generated by Python)
# Available files:
#   - {model_name}_{scenario}_coefficients.csv
#   - {model_name}_{scenario}_wtp.csv  
#   - {model_name}_{scenario}_summary.txt
...
# Load data (use master_data_with_travel_cost)
# The Python script handles all the preprocessing
data_clean <- read_parquet(inputs$master_data_with_travel_cost)
setDT(data_clean)
...
if (!file.exists(outputs$voronoi_plot)) {
  ...
  output_dir <- dirname(outputs$voronoi_plot)
...
# Ensure output directory exists
summary_output_dir <- dirname(outputs$data_summary)
